...
type StorageWithActualPath struct {
	Storage    driver.Driver
	ActualPath string
}

func GetStoragesAndActualPath(rawPath string) ([]StorageWithActualPath, error) {
	...
	results := make([]StorageWithActualPath, 0, len(storages))
	for _, storage := range storages {
		mountPath := utils.GetActualMountPath(storage.GetStorage().MountPath)
		results = append(results, StorageWithActualPath{
			Storage:    storage,
