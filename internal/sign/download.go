...
func SignDownload(user *model.User, path, ip string) string {
	username := ""
	if user != nil {
		username = user.Username
	}
	if !setting.GetBool(conf.SignBindIP) {
		ip = ""
	}
...
func VerifyDownload(path, ip, token string) (*model.User, error) {
	if strings.HasPrefix(token, downloadTokenPrefix) {
		return verifyDownloadV2(path, ip, token)
	}
	return nil, Verify(path, token)
}

func verifyDownloadV2(path, ip, token string) (*model.User, error) {
	...
	if setting.GetBool(conf.SignBindIP) && payload.IP != "" && payload.IP != ip {
		return nil, pkgSign.ErrSignInvalid
	}
	if payload.User == "" {
		return nil, nil
...
// ExtractUsernameFromDownloadSign verifies the download token and extracts the username
// without performing any DB lookup. Returns the username (empty for guest) and any error.
func ExtractUsernameFromDownloadSign(path, ip, token string) (string, error) {
	if !strings.HasPrefix(token, downloadTokenPrefix) {
		// Legacy token format - verify only, no username embedded
		if err := Verify(path, token); err != nil {
			return "", err
		}
		return "", nil // Guest for legacy tokens
	}

	token = strings.TrimPrefix(token, downloadTokenPrefix)
	parts := strings.Split(token, ".")
	if len(parts) != 2 {
		return "", pkgSign.ErrSignInvalid
	}
	payloadEnc := parts[0]
	sig := parts[1]
	if !hmac.Equal([]byte(sig), []byte(signPayload(payloadEnc, downloadSecret()))) {
