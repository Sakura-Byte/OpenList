...
func (d *Teambition) chunkUpload(ctx context.Context, file model.FileStreamer, token string, up driver.UpdateProgress) (*FileUpload, error) {
	...
	_, err := base.RestyFor(d).R().SetResult(&newChunk).SetHeader("Authorization", token).
		SetBody(base.Json{
	...
	for i := 0; i < newChunk.Chunks; i++ {
		...
		log.Debugf("url: %s", u)
		_, err := base.TransferRestyFor(d).R().
			SetContext(ctx).
	...
	_, err = base.TransferRestyFor(d).R().SetHeader("Authorization", token).Post(
